...
- Capabilities are plain class attributes: `initials`, `language` (`"de"`, `"en"`, or `"fr"`), `supports_imdb`, `supports_phrase`, `supported_categories` (constants `SEARCH_CAT_*`), plus optional `supports_absolute_numbering` / `requires_login` / `requires_account` / `invite_only` / `requires_flaresolverr` / `feed_requires_radarr` / `feed_requires_sonarr` (the latter two are feed-only — the setup wizard prompts for the client, but ID search never needs it and `feed` degrades with a warning when it is missing). `language`, category, account, invite, login, FlareSolverr, and feed-client metadata is surfaced in the hostname editor through `helpers.get_source_metadata()`.
