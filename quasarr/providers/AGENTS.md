...
- `download_package` derives the JD `destinationFolder` from env: base folder `DOWNLOAD_FOLDER` (default `Quasarr`, may be emptied or absolute) plus, when `CATEGORY_SUBFOLDERS` is truthy, the package's download category via `storage.categories.get_download_category_from_package_id` (unknown/`not_quasarr` packages stay base-only). With neither set it stays `Quasarr/<jd:packagename>`.
